...
const tsToLocal = (ts) => (ts ? new Date(Number(ts) * 1000).toLocaleString() : "-");
...
export default function PacksByWalletPage() {
  ...
  const [source, setSource] = useState("auto"); // auto|polygonscan|rpc
  ...
  const Card = ({ title, children }) => (
    <div style={{ background: "#12151a", border: "1px solid #232a34", borderRadius: 16, padding: 16, marginTop: 16 }}>
      <h3 style={{ margin: 0, marginBottom: 8, fontSize: 16 }}>{title}</h3>
      <div style={{ fontSize: 13, lineHeight: 1.4 }}>{children}</div>
    </div>
  );
  const Badge = ({ text }) => (
    <span style={{ background: "#16351f", border: "1px solid #2b7a3f", color: "#7df1a9", padding: "2px 8px", borderRadius: 999, fontWeight: 700, fontSize: 12, marginLeft: 8 }}>
      {text}
    </span>
  );

  return (
    <div style={{ minHeight: "100vh", background: "#0b0f14", color: "#e7ecef", fontFamily: 'ui-sans-serif, system-ui, -apple-system, Segoe UI, Roboto, "Helvetica Neue", Arial', padding: 24, maxWidth: 1200, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "flex-end" }}>
        <div style={{ flex: "1 1 auto", minWidth: 420 }}>
          ...
          <input value={wallet} onChange={(e) => setWallet(e.target.value)} placeholder="0x…" spellCheck={false}
            style={{ width: "100%", background: "#0e141b", border: `1px solid ${!wallet || isAddr(wallet) ? "#1f2630" : "#7a2b2b"}`, borderRadius: 10, color: "#e7ecef", padding: "10px 12px", fontSize: 14, outline: "none" }} />
        </div>
        ...
        <div style={{ width: 140 }}>
          ...
          <input type="number" min={1} max={500} value={limit} onChange={(e) => setLimit(Number(e.target.value || 50))}
            style={{ width: "100%", background: "#0e141b", border: "1px solid #1f2630", borderRadius: 10, color: "#e7ecef", padding: "10px 12px", fontSize: 14, outline: "none" }} />
        </div>

        <div style={{ width: 180 }}>
          <div style={{ fontSize: 12, opacity: 0.75, marginBottom: 6 }}>Source</div>
          <select value={source} onChange={(e) => setSource(e.target.value)}
            style={{ width: "100%", background: "#0e141b", border: "1px solid #1f2630", borderRadius: 10, color: "#e7ecef", padding: "10px 12px", fontSize: 14 }}>
        ...
        <div style={{ width: 160 }}>
          <div style={{ fontSize: 12, opacity: 0.75, marginBottom: 6 }}>Start block (opt.)</div>
          <input type="number" value={startblock} onChange={(e) => setStartblock(e.target.value)} placeholder="ex: 74000000"
            style={{ width: "100%", background: "#0e141b", border: "1px solid #1f2630", borderRadius: 10, color: "#e7ecef", padding: "10px 12px", fontSize: 14 }} />
        </div>

        <div style={{ width: 160 }}>
          <div style={{ fontSize: 12, opacity: 0.75, marginBottom: 6 }}>End block (opt.)</div>
          <input type="number" value={endblock} onChange={(e) => setEndblock(e.target.value)} placeholder="ex: 76000000"
            style={{ width: "100%", background: "#0e141b", border: "1px solid #1f2630", borderRadius: 10, color: "#e7ecef", padding: "10px 12px", fontSize: 14 }} />
        </div>

        <button onClick={onScan} disabled={loading || !isAddr(wallet)}
          style={{ height: 40, background: loading ? "#2b3240" : "#1c7cf0", border: "none", borderRadius: 10, color: "white", padding: "0 16px", fontSize: 14, cursor: loading ? "not-allowed" : "pointer", fontWeight: 700 }}
          title={!isAddr(wallet) ? "Wallet invalide" : "Scanner"}>
          {loading ? "Scan…" : "Scanner"}
        </button>
      </div>

      {error && (<Card title="Erreur"><div style={{ color: "#ff6b6b" }}>{error}</div></Card>)}

      {res && res.ok && (
        <>
          <Card title="Résumé">
            <div style={{ display: "grid", gridTemplateColumns: "220px 1fr", rowGap: 6 }}>
              <div style={{ opacity: 0.7 }}>Wallet</div><div style={{ fontFamily: "monospace" }}>{res.wallet}</div>
              <div style={{ opacity: 0.7 }}>Source</div><div>{res.scans.source}{res.scans?.debug?.hasPolygonscanKey ? " (clé OK)" : ""}</div>
              <div style={{ opacity: 0.7 }}>Candidats analysés</div>
              <div>{res.scans.analyzed} (sur {res.scans.candidates})<Badge text={`${res.scans.packsDetected} packs`} /></div>
              <div style={{ opacity: 0.7 }}>Range (RPC)</div>
              <div>{res.scans.range ? `${res.scans.range.startblock} → ${res.scans.range.endblock}` : "—"}</div>
              <div style={{ opacity: 0.7 }}>Total packs</div><div>{res.totals.packs}</div>
              <div style={{ opacity: 0.7 }}>Total dépensé (USDC)</div><div>{fmt(res.totals.spentUSDC)}</div>
              <div style={{ opacity: 0.7 }}>Prix moyen / pack (USDC)</div><div>{res.totals.unitPriceAvgUSDC != null ? fmt(res.totals.unitPriceAvgUSDC, 6) : "-"}</div>
              <div style={{ opacity: 0.7 }}>Influence totale</div><div>{res.totals.influence}</div>
            </div>
          </Card>
                ...
                <tbody>
                  {res.items.map((it, i) => (
                    <tr key={i}>
                      ...
                      <td style={{ padding: 6 }}>
                        <a href={`https://polygonscan.com/tx/${it.txHash}`} target="_blank" rel="noreferrer"
