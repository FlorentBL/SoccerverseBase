...
const RINCON_URL = "/rincon_mapping.json"; // hébergé local
...
export default function SoccerverseScouting() {
  ...
  // Chargement JSON externe une seule fois
  ...
  return (
    <div style={{
      ...
      <h2 style={{ fontWeight: 800, fontSize: 28, marginBottom: 24, letterSpacing: 1 }}>Scouting Soccerverse (détail)</h2>
      <div style={{
        background: "#23272e", padding: 28, borderRadius: 12, boxShadow: "0 2px 12px #0008", minWidth: 340, maxWidth: 520,
      }}>
        ...
        {playerInfo && (
          <div style={{ marginTop: 28, background: "#181d23", borderRadius: 10, padding: 18, boxShadow: "0 2px 8px #0003" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", color: "#eee", fontSize: 16 }}>
              <tbody>
                <tr><td style={{ fontWeight: 700, padding: 5 }}>Nom</td><td style={{ padding: 5 }}>{playerInfo.name || <span style={{ color: "#ff6" }}>Non dispo</span>}</td></tr>
                ...
                <tr><td style={{ fontWeight: 700, padding: 5 }}>Valeur</td><td style={{ padding: 5 }}>{playerInfo.value ? playerInfo.value.toLocaleString() : "-"}</td></tr>
                <tr><td style={{ fontWeight: 700, padding: 5 }}>Salaire</td><td style={{ padding: 5 }}>{playerInfo.wages ? playerInfo.wages.toLocaleString() : "-"}</td></tr>
                <tr><td style={{ fontWeight: 700, padding: 5 }}>Dernier prix</td><td style={{ padding: 5 }}>{playerInfo.last_price || "-"}</td></tr>
