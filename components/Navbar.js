...
export default function Navbar() {
  ...
  // Détecte la langue dans l’URL (default : fr)
  const currentLang = LANGS.find(l => pathname.startsWith("/" + l.code))?.code || "fr";
  const menuItems = MENU_LABELS[currentLang];

  // Construit le href correct pour une page dans une autre langue
  function getLangHref(targetLang) {
    ...
    // Si la route est "/" ou autre (sans langue), on préfixe
    return "/" + targetLang + pathname;
  }

  // Préfixe les liens internes par la langue
    ...
    <header className="fixed top-0 left-0 w-full z-50 backdrop-blur-md bg-white/10 border-b border-white/20 text-white">
      <nav className="flex flex-col items-center max-w-7xl mx-auto py-3 px-4 relative">
        ...
        <div className="hidden md:flex gap-8 text-base font-bold items-center justify-center">
          ...
          {/* Switch langues (boutons groupés) */}
          <div className="ml-4 flex gap-1">
            {LANGS.map(l =>
              <button
                key={l.code}
                onClick={() => window.location.href = getLangHref(l.code)}
                disabled={currentLang === l.code}
        ...
        <div className="md:hidden bg-gray-900/80 backdrop-blur-md border-b border-white/10 px-4 pb-4">
          ...
          <div className="flex justify-center mt-2 mb-3">
            <a
              ...
              title="Buy me a coffee"
            >
              <div className="p-1 rounded-full bg-yellow-300/80 hover:bg-yellow-400 transition">
                {/* ...SVG identique à celui du menu desktop... */}
          ...
          {/* Switch langues (mobile) */}
          <div className="flex justify-center mt-2 mb-3 gap-1">
            {LANGS.map(l =>
              <button
                key={l.code}
                onClick={() => { setMenuOpen(false); window.location.href = getLangHref(l.code); }}
                disabled={currentLang === l.code}
                className={`text-sm px-3 py-1 rounded ${
                  currentLang === l.code
                    ? "bg-green-600 text-white font-bold"
